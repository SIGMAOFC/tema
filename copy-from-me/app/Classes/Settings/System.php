...
class System
{
    ...
    public function checkPteroClientkey()
    {
        ...
        if ($response->failed()) {
            return redirect()
                ->back()
                ->with('error', __('Your Key or URL is not correct'));
        }

        return redirect()
            ->back()
            ->with('success', __('Everything is good!'));
    }
    ...
    public function updateSettings(Request $request)
    {
        ...
        if ($validator->fails()) {
            return redirect(route('admin.settings.index') . '#system')
                ->with('error', __('System settings have not been updated!'))
