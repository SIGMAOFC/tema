      ...
      <form action="{{ route('admin.store.update', $shopProduct->id) }}" method="POST">
        ...
        <x-label title="Type">
          <select required name="type" id="type"
            ...
            <option @if ($shopProduct->type == 'credits') selected @endif value="Credits">
              {{ CREDITS_DISPLAY_NAME }}</option>
            <option @if ($shopProduct->type == 'Server slots') selected @endif value="Server slots">
        ...
        <x-label title="Price">
          <x-input value="{{ $shopProduct->price }}" id="price" name="price" type="number" placeholder="10.00"
            step="any"></x-input>
        </x-label>
        <x-label title="Quantity">
          <x-input value="{{ $shopProduct->quantity }}" id="quantity" name="quantity" type="number"
            placeholder="1000"></x-input>
          <x-slot name="text">{{ __('Amount given to the user after purchasing') }}</x-slot>
        </x-label>
        <x-label title="Display">
          <x-input value="{{ $shopProduct->display }}" id="display" name="display" type="text"
        ...
        <x-label title="Description">
          <x-input value="{{ $shopProduct->description }}" id="description" name="description" type="text"
